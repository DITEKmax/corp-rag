...
- LLM: бесплатные и условно-бесплатные tier'ы (Gemini Free, OpenRouter `:free`).
...
| LLM clients | google-generativeai + openai (для OpenRouter) | Gemini + OpenRouter |
...
|---|---|---|
| Генерация ответа | Gemini 2.0 Flash Lite | Free tier 1500/день, 1M контекст, дешёвая |
| Fallback генерации | DeepSeek V3 на OpenRouter `:free` | На случай rate limit Gemini |
| Entity/relation extraction (граф) | Gemini 2.0 Flash (не Lite) | Лучше reasoning; выполняется редко (только при индексации) |
| Query router | `deepseek/deepseek-chat-v3-0324:free` | Структурированный JSON, быстро |
| Input guard classifier | DeepSeek V3 | Экономия |
...
│   │   │   └── entity_extractor.py      (Gemini Flash)
│   │   ├── retrieval/
...
│   │   ├── generation/
│   │   │   ├── llm_client.py            (Gemini + OpenRouter)
│   │   │   ├── prompts.py               (системные промпты с XML)
│   │   │   └── synthesizer.py           (Gemini + citations)
│   │   └── guards/
...
| `synthesis` | `query`, `ranked_chunks` | `answer`, `citations`, `confidence`, `answered` | Gemini Flash Lite с XML-prompt, structured output |
...
- JWT secret, OpenRouter key, Google key, MinIO credentials — всё через env.
...
- Для каждого parent chunk → Gemini 2.0 Flash с structured output (`Extraction { entities, relations }`).
...
- DeepSeek V3 с JSON mode + few-shot (2–3 примера на класс).
...
#### 11.4.2 Structured output (Gemini)
```python
...
| 10.8 | `EntityExtractor` (Gemini Flash, structured output) | 0.5 |
...
| 14.3 | Streaming-ответ Gemini через SSE на фронт или ждём целиком? | Целиком в MVP, SSE можно потом |
